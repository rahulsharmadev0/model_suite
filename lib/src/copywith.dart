...
final _copywithMacro = Uri.parse('package:model_suite/src/copywith.dart');
...
class CopyWithModelBuilder extends ModelBuilder {
  ...
  Future<void> build() async {
    ...
    if (!clazzData.hasConstructor) {
      ...
      // Create a mapping of all fields including inherited ones
      var fieldMapping = <String, FieldDeclaration>{};

      // Add fields from current class
      for (var field in clazzData.clazzfields) {
        fieldMapping[field.identifier.name] = field;
      }

      // Add fields from superclass if exists
      for (var field in clazzData.superClazzFields) {
        fieldMapping[field.identifier.name] = field;
      }

      final constructorParameters = clazzData.constructor!.parameters;
      for (var cParam in constructorParameters) {
        final paramName = cParam.identifier.name;
        var field = fieldMapping[paramName];

        // If we can't find the field but have a type in the constructor parameter, use that
        if (field == null && cParam.type is NamedTypeAnnotation) {
          params.add(Parameter.fromFPD(cParam));
        } else if (field != null) {
          params.add(Parameter.fromBothFieldAndFPD(field, cParam));
        } else {
          throw exception.missingTypeAnnotationsError(className: paramName);
        }
      }
    }
